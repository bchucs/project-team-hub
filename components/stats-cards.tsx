...
function StatCard({ value, label, highlight }: StatCardProps) {
  return (
    <div className="rounded-lg bg-card border border-border p-6 space-y-2">
      <p className={`heading-3 ${highlight ? "text-primary" : "text-foreground"}`}>
        {value}
      </p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="heading-3">{stats.cycleName}</h1>
          {stats.deadline && (
